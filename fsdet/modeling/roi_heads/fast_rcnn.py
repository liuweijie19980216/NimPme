...
class AnomalyFastRCNNOutputs(object):
    ...
    def __init__(
        ...
        if proposals[0].has("gt_boxes"):
            ...
            # base_gt_class_boad = self.gt_classes.view(-1, 1).expand(len(self.gt_classes),
            #                                                         len(self.base_classes_conti))
            # bg_gt_class_boad = self.gt_classes.view(-1, 1).expand(len(self.gt_classes),
            #                                                       len(self.bg_classes_conti))

            novel_class_boad = self.novel_classes_conti.view(1, -1).expand(len(self.gt_classes), len(self.novel_classes_conti)).cuda()
            # base_class_boad = self.base_classes_conti.view(1, -1).expand(len(self.gt_classes),
            #                                                                len(self.base_classes_conti)).cuda()
            # bg_class_boad = self.bg_classes_conti.view(1, -1).expand(len(self.gt_classes),
            #                                                          len(self.bg_classes_conti)).cuda()
            self.novel_ids = (novel_gt_class_boad == novel_class_boad).nonzero()[:, 0]
            # self.base_ids = (base_gt_class_boad == base_class_boad).nonzero()[:, 0]
            # self.bg_ids = (bg_gt_class_boad == bg_class_boad).nonzero()[:, 0]
            self.base_bg_ids = ((novel_gt_class_boad == novel_class_boad).sum(dim=1) == 0).nonzero().view(-1,)

            self.is_pseudo = []
            for p in proposals:
                self.is_pseudo.extend(p.get('is_pseudo'))
            self.is_pseudo = torch.tensor(self.is_pseudo)
            keep_pseudo = self.is_pseudo[self.novel_ids].cuda()
            self.novel_pseudo_id = self.novel_ids[keep_pseudo]
            if len(self.novel_pseudo_id) == 0:
                print(self.is_pseudo)
            self.true_id = self.is_pseudo == False
            keep_true = self.true_id[self.novel_ids].cuda()
            self.novel_true_id = self.novel_ids[keep_true]
    ...
    def anomaly_loss(self):
        ...
        novel_classes = self.gt_classes[self.novel_pseudo_id]
        novel_ce = loss_cross_entropy[self.novel_pseudo_id]
        for novel_id, cls in enumerate(novel_classes):
            class_loss[str(cls.item())].append(novel_ce[novel_id])
    ...
    def softmax_cross_entropy_loss(self, features=None):
        ...
        pred_class_logits_base = self.pred_class_logits[self.base_bg_ids]
        pred_class_logits_novel_true = self.pred_class_logits[self.novel_true_id]
        pred_class_logits_true = torch.cat([pred_class_logits_base, pred_class_logits_novel_true], dim=0)
        label_base = self.gt_classes[self.base_bg_ids]
        label_novel_true = self.gt_classes[self.novel_true_id]
        label_novel_pseudo = self.gt_classes[self.novel_pseudo_id]
        label_true = torch.cat([label_base, label_novel_true], dim=0)
        cross_entropy_true = F.cross_entropy(
            pred_class_logits_true, label_true, reduction="mean"
        )

        pred_class_logits_novel_pseudo = self.pred_class_logits[self.novel_pseudo_id]

        if ce_mean:
            cross_entropy_novel_pseudo = F.cross_entropy(
                pred_class_logits_novel_pseudo, label_novel_pseudo, reduction="none"
            )
            novel_mean = cross_entropy_novel_pseudo.mean()
            cross_entropy_novel_pseudo = cross_entropy_novel_pseudo[cross_entropy_novel_pseudo < novel_mean].mean()


        return cross_entropy_true*0.9 + cross_entropy_novel_pseudo*0.1
    ...
    #         fg_gt_classes_novel = gt_classes_base[fg_inds_novel]
    #         # pred_proposal_deltas for class k are located in columns [b * k : b * k + b],
    #         # where b is the dimension of box representation (4 or 5)
    #         # Note that compared to Detectron1,
    #         # we do not perform bounding box regression for background classes.
    ...
    def smooth_l1_loss(self):
        ...
        gt_proposal_deltas_true = gt_proposal_deltas[self.true_id]
        ...
        gt_classes_true = self.gt_classes[self.true_id]

        # fg_inds = torch.nonzero(
        #     (self.gt_classes >= 0) & (self.gt_classes < bg_class_ind)
        # ).squeeze(1)
        fg_inds_true = torch.nonzero(
            (gt_classes_true >= 0) & (gt_classes_true < bg_class_ind)
        ).squeeze(1)
        ...
        if cls_agnostic_bbox_reg:
            ...
            #fg_gt_classes = self.gt_classes[fg_inds]
            fg_gt_classes_true = gt_classes_true[fg_inds_true]
            ...
            #     box_dim, device=device
            # )
            gt_class_cols_true = box_dim * fg_gt_classes_true[:, None] + torch.arange(
                box_dim, device=device
            )
        ...
        #     reduction="sum",
        # )
        pred_proposal_deltas_true = self.pred_proposal_deltas[self.true_id]

        loss_box_reg_true = smooth_l1_loss(
            pred_proposal_deltas_true[fg_inds_true[:, None], gt_class_cols_true],
            gt_proposal_deltas_true[fg_inds_true],
            self.smooth_l1_beta,
            reduction="sum",
        )
        ...
        # in minibatch (2) are given equal influence.
        # loss_box_reg = loss_box_reg / self.gt_classes.numel()
        loss_box_reg_true = loss_box_reg_true / gt_classes_true.numel()

        return loss_box_reg_true # + loss_box_reg_novel

    def losses(self, features=None):
        """
        Compute the default losses for box head in Fast(er) R-CNN,
        with softmax cross entropy loss and smooth L1 loss.
